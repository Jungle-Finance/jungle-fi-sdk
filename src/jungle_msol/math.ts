...
export class MsolVaultMath extends GenericVaultMath<VaultInfo> {
    ...
    async calcDepositReturns(
        ...
        const currentExchangeRate = JSBI.BigInt(
            Math.floor(LAMPORTS_PER_MEGASOL / msolPriceFromState)
        );

        // calc returnAmount = amount * exchangeRate / initialExchangeRate
        const returnAmount = JSBI.divide(
            JSBI.multiply(amount, currentExchangeRate),
            initialExchangeRate
        );
